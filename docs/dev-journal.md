...
## 2026-04-20 — Second wave: 10 PRs landed (CI fix + POL-01 research + AUDIT-03/04/12 + LOOT-01 + AI-SAFETY-01 + COMBAT-01a) + a hard lesson on UI verification

A second wave of 10 PRs (#43 → #52) landed after the overnight sprint entry below. Two open PRs remain at the end of the day: **#53 (SPEC-UI-FONT-01)** and **#54 (COMBAT-01b)**, both CI green but awaiting PO visual verification before merge. The hard lesson of the day was codified in [AGENTS.md §0b](../AGENTS.md#0b-ui-verification-windowed-not-headless-non-negotiable): I don't get to claim "looks good" without actually running Godot in a window, and the PO caught me doing exactly that on #49/#53/#54.

**What landed, in merge order:**

- **#43 · tracker flip overnight audit merges to Done + sprint journal** (docs-only) — brought the tracker into sync with the nine overnight merges so the next day's work wasn't navigating from stale "To Do" state.
- **#44 · CI fix: export GODOT_BIN for e2e-tests job** (`.github/workflows/ci.yml`) — every main-push E2E / GdUnit4 run had been failing with `Godot runtime is not configured. The environment variable 'GODOT_BIN' is not set or empty.`, which also gated the Sandbox Headless Checks job (`needs: e2e-tests`). `chickensoft-games/setup-godot@v2` exports `GODOT` and `GODOT4` but not the `GODOT_BIN` that GdUnit4's dotnet-test runner specifically reads. One-line env block pointing `GODOT_BIN` at `/home/runner/bin/godot` unblocked the job — which immediately surfaced the next failure stage (AUDIT-18 scene-bootstrap timeout, see AUDIT-17 re-triage below).
- **#45 · POL-01 audio research + license policy + asset inventory** (docs-only) — PO reopened POL-01 as a research task: pick OGA tracks + write a license-discipline guide. New [docs/assets/audio-pipeline.md](assets/audio-pipeline.md) locks §1 license policy (CC0 / OGA-BY preferred; CC-BY flagged for DRM-platform review; CC-BY-SA / CC-BY-NC / CC-BY-ND / GPL rejected), §2 directory contract + CREDITS protocol, §3 music inventory (9 tracks), §4 SFX inventory (46 files), §5 curated OGA shortlist (5 music + 7 SFX), §6 wiring plan (MusicManager + SfxManager autoloads + bus architecture), §8 acceptance criteria. Impl deferred until PO approves the shortlist.
- **#46 · AUDIT-03 `SaveManager` null-slot guard** — `Save`, `SaveToSlot`, and `LoadSlot` refuse (with `GD.PrintErr`) when `CurrentSaveSlot` is null instead of falling through to slot 0. PR #9 had fixed the `ClassSelect.Reset()` path but the audit found two more sites that null `CurrentSaveSlot` before a save fires (PauseMenu "Back to Main Menu", DeathScreen quit). Option (a) — refuse at the boundary — is safer than auditing every caller, and future refactors that re-introduce the pre-null autosave are now a logged no-op rather than a silent corruption.
- **#47 · AUDIT-04 `MagiculeAttunement.ImportState` point filter** — `TotalPoints` was reconstructed from `_clearedFloors.Count` without filtering by `> UnlockFloor`, so corrupt or pre-unlock-era saves could inflate the player's attunement points. Fix rebuilds `TotalPoints` by iterating `_clearedFloors` with the unlock filter, mirroring the live award path in `OnFloorCleared`. One new test pins the pre-unlock-floor drop case.
- **#48 · AUDIT-12 `DungeonIntelligence` test coverage** — the 199-LOC `DungeonIntelligence.cs` was completely untested. New `tests/unit/DungeonIntelligenceTests.cs` adds 18 tests covering time-based decay via `Update(dt)`, pressure-score thresholds (low / med / high), and modifier curves at empty / typical / saturated pressure. Future endgame work can refactor the decay/pressure math with a regression net.
- **#49 · LOOT-01 Container.cs + ContainerLootTable + spawn integration** (SPEC-LOOT-01) — equipment is now container-exclusive; monsters keep gold + XP + materials. New `scripts/logic/ContainerLootTable.cs` (pure logic) with per-type tables (Jar / Crate / Chest), floor-linear gold scaling `× (1 + floor × 0.05)`, zone-tilted signature materials, slot-uniqueness within a container, spawn-count scaling `+1 / 10 floors`. New `scripts/Container.cs` — StaticBody2D + Area2D interact (mirroring `Npc.cs`), placeholder ColorRect visuals (ART-15 replaces), single-use open + desaturate `Modulate`. `Dungeon.SpawnContainers()` runs after `SpawnInitialEnemies` at both initial load and floor change; containers clear alongside enemies on floor transition via new `Constants.Groups.Containers`. `MonsterDropTable.RollEquipment` + `CandidatesForTier` removed; `Enemy.cs` no longer rolls equipment; three `RollEquipment_*` tests deleted. New `ContainerLootTableTests` (19 tests) covers spawn-count scaling, gold range + scaling, generic material tier/count, signature zone-tilt, jar small-thing pool, chest/crate slot-uniqueness, end-to-end `Roll`.
- **#50 · SPEC-AI-SAFETY-01 generative-AI safety policy** (docs-only, legal compliance) — PO directive responding to new generative-AI regulatory requirements. [docs/conventions/ai-safety-policy.md](conventions/ai-safety-policy.md) locks: §1 prohibited-content taxonomy (7 hard-reject categories + narrow nudity allowance + gray-zones escalate to PO), §2 dev-time guardrails (prompt + output classification + provenance audit trail at `docs/assets/ai-provenance.ndjson`), §3 runtime guardrails (future — MVP has no runtime AI), §4 in-app reporting (F10 global + per-surface context menu; local-only in MVP via `user://ai_reports.ndjson`, cloud opt-in post-launch), §5 weekly triage + filter-update loop, §6 compliance-by-law table, §7 audit artifacts list, §8 roles, §10 five impl follow-ups (Report UI, credits sweep, triage stub, pipeline-spec back-refs, provenance log init). **MVP blocker:** POL-AI-REPORT-UI-01 must ship in the first build containing any player-visible AI-generated surface.
- **#51 · COMBAT-01a pure-logic foundation** (SPEC-COMBAT-EQUIPMENT-INTEGRATION-01) — the spec's test-first layer. New `scripts/logic/CombatFormulas.cs` with `SoftCap(raw) = raw * (60 / (raw + 60))` and all four overflow conversions (Crit → damage-multiplier bonus, Haste → Flurry free-extra-swing chance capped 40%, Dodge → Phase i-frame duration capped 500 ms, Block → reduction above 50% baseline capped 80% total). New `EquipmentCombatStats` struct on `EquipmentSet` cache carries raw crit/haste/dodge/block %. New `RecomputeDerivedStats()` unifies the four MaxHp recompute sites. No runtime wiring — the formulas return values that nothing consumes yet, deliberately. The whole pure-logic layer ships behind a comprehensive test net so #54 can land runtime wiring without also landing new formula surface area.
- **#52 · AUDIT-17 re-triage + AUDIT-18 new row** (docs-only) — AUDIT-17's original `MissingMethodException` for `FileAccess.GetAsText` / `CanvasItem.DrawString` was caused by a Godot binary vs GodotSharp NuGet version mismatch; PR #26 (`d796419`, Godot 4.6.2 sync) resolved it, and UI Tests have been green on every main push since. Flipped to Done (re-triaged). The distinct post-GODOT_BIN-fix failure — `System.TimeoutException` inside `GodotGdUnit4RestClient` during scene bootstrap — tracked as new **AUDIT-18** (P2, since UI Tests covers the keyboard-nav surface and E2E is still scaffolded-not-wired per TEST-04).

**Still open at end of day:**

- **#53 · SPEC-UI-FONT-01 (Press Start 2P)** — round-5 Copilot fixes pushed. CI green. Awaiting PO visual verification.
- **#54 · COMBAT-01b runtime wiring** — round-1 Copilot fixes pushed. CI green. Awaiting PO visual verification. `GameState.TakeDamage` now consumes the four overflow conversions via the spec's Phase→Dodge→Block resolution order; `Player.ExecuteAttackImpl` overlays equipment stats before damage math; Flurry free-extra-swing wired; `GameState.CombatRng` + Phase state reset on `ResetForNewGame`; MISS / BLOCK / PHASED floating-text variants added.

**The hard lesson: "UI verification" means *actually running Godot in a window*, not "tests pass locally."**

The PO caught me claiming "PS2P renders crisp," "containers work visually," and "MISS/BLOCK/PHASED fires correctly" on three separate PRs without ever running the game in a window. I'm an agent in a sandbox — I can't see pixels. Unit tests prove logic; they don't prove a float clips wrong on Retina or that the font loads at integer scale. The rule is now codified in [AGENTS.md §0b](../AGENTS.md#0b-ui-verification-windowed-not-headless-non-negotiable) and [CLAUDE.md hard rule #9](../CLAUDE.md): UI verification is windowed, never headless. `make test-ui` is already windowed by default per Makefile; `make run` is the manual path; `--headless` is CI-only. Three PRs (#49, #53, #54) carry the debt of not having had eyeball verification at merge time — PO verification is now the gate before any UI-touching PR lands. Saved as [feedback_no_headless_ui_tests.md](../../.claude/projects/-Users-johndilig-Projects-infinite-dungeon-game/memory/feedback_no_headless_ui_tests.md) but the rule itself lives in the shared docs so every agent sees it, not just me.

**The secondary lesson: write rules to shared docs, not private memory.** After the PO's first pushback on headless I wrote the rule to my Claude-only memory — which no other agent reads, and which the PO correctly called out: "you stuck it in memory, but where did you write it? nowhere?" Memory is a private reminder; the rule belongs in AGENTS.md + CLAUDE.md. Fixed same session.

**Dev-tracker rows flipped to Done in this cleanup pass:** AUDIT-03, AUDIT-04, AUDIT-12 — three rows that shipped as #46/#47/#48 but were still "To Do." COMBAT-01 row split to reflect 01a Done / 01b in review instead of a single "Ready-for-impl" that no longer matched reality.

**Why this journal entry is one-paragraph-per-PR instead of the overnight sprint's drill-down style:** the overnight sprint entry below was the deep-dive because that session was dominated by Copilot-round churn and had two durable process lessons (no PR-round refs; local time only). The second wave was more linear — PRs merged without pushing me against the same Copilot review pattern multiple times, the one durable lesson (windowed UI verification) is codified in shared docs rather than bottled up in this journal, and the interesting stuff is the shape of the ticket graph collapsing rather than any one PR's mechanics.
