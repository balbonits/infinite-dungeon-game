...
| TEST-11 | CI workflow failing at parse — no jobs dispatch | To Do | P1 | Every push-event CI run since mid-April reports `failure` with zero jobs dispatched and `workflowName` returned as the file path (`.github/workflows/ci.yml`) rather than the declared `name:` field — GitHub falls back to path when the workflow fails preflight validation. PRs #3..#8 all merged with the same red state. YAML itself parses cleanly and no encoding issues are visible. Next step: open a throwaway branch that trims `ci.yml` to a single no-op job, push, and binary-search back toward the full config to isolate which directive GitHub is rejecting. |
...
| SPEC-ISO-01 | Isometric rendering conversion — system spec | Needs-spec | P1 | **Major architectural pivot.** Current game uses 32×32 top-down tiles; moving to isometric tiles (Diablo-style 2:1 diamond projection, typical cell 64×32). Spec must cover: tile geometry (cell dimensions, anchor points, height slot for walls/objects), screen-to-world coordinate math, Z-ordering rule (y-sort + object height), input-to-movement mapping (WASD → iso diagonals or cardinal?), camera follow in iso space, collision shapes (diamond AABB or polygon?), wall occlusion when player is behind a tall tile, sprite atlas layout per biome, and migration path (can we ship iso tiles in parallel with top-down, or is it an all-or-nothing cutover?). Cross-refs: existing `Dungeon.cs`, `Player.cs`, `Enemy.cs`, `Main.cs` scene loading. |
| ISO-01 | Isometric rendering conversion — implementation | Blocked on SPEC-ISO-01 | P1 | Implement the spec. Rewrites `Dungeon.cs` tile placement, adds iso coordinate helpers, updates `Player.cs` + `Enemy.cs` movement + sprite orientation, updates `Camera2D` follow math, adds y-sort for depth ordering, reworks collision shapes for iso cells, updates all existing scenes. **This is multi-session scope** — not a single-branch PR. Likely broken into: ISO-01a coord math + helpers, ISO-01b Dungeon.cs rewrite, ISO-01c Player/Enemy movement, ISO-01d Camera + Y-sort, ISO-01e scene migration, ISO-01f input remap. Existing top-down tile assets retained as legacy until iso set ships. |
