...
| SPEC-ISO-01 | Isometric rendering conversion — system spec | Draft (PO review) | P1 | **Spec drafted:** [docs/systems/iso-rendering.md](systems/iso-rendering.md). Locks 64×32 floor diamonds, 64×64 walls, anchor at bottom-center; world↔screen formulas with worked examples; Y-sort via Godot's built-in container; WASD = visual cardinal (Diablo-style); circle colliders kept for entities, diamond polygon for walls; v1 wall occlusion = alpha-modulate the 3 cells visually-above the player; per-scene `Constants.Rendering` flag for migration. ISO-01 sub-phases (a–f) sequenced. Touch surface enumerated for 9 code files + 5 scenes + 5 downstream docs. Awaiting PO review before ISO-01 unblocks. |
