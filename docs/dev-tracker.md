...
| AUDIT-15 | Coverage Gate broken — empty coverage XML on every CI run | To Do | P2 | Coverage Gate has been failing on every PR since TEST-11 (PR #10) introduced it. Local repro: `coverage.opencover.xml` is generated but contains zero modules (`<Modules />`, `numClasses="0"`). Root cause: the test project compiles production source files in directly via `<Compile Include="../../scripts/logic/*.cs" />` rather than referencing `DungeonGame.csproj`, so coverlet's `XPlat Code Coverage` collector has no separate production assembly to instrument. The `+DungeonGame` assembly filter is downstream of the empty data. Fix options: (a) restructure `tests/unit/` to reference a separate `DungeonGame.Logic.csproj` containing only pure-logic source, (b) switch to `coverlet.msbuild` with explicit Include filters that target the compiled-in source paths, or (c) accept that coverage is for the test assembly itself and adjust `+DungeonGame*` filter + threshold accordingly. Blocks reliable CI gating on real-code PRs. |
