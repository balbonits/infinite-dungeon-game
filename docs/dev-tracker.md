...
| ART-SPEC-01 | PixelLab prompt template library | Done (spec) | P1 | Locked in [docs/assets/prompt-templates.md](assets/prompt-templates.md). Named blocks: **CHAR-HUM-STD** (humanoid characters / player classes / NPCs), **CHAR-MON-VAR** (monsters — with `.quad` / `.wing` / `.arach` / `.biped-mon` sub-variants), **TILE-ISO-FLOOR-WALL** (isometric floor + wall tiles, per-biome palette sub-clamps for 8 biomes), **OBJ-MAP** (chests / furniture / stairs / props with size reference table), **ICON-UI-64** (64×64 ability/skill/item icons with shared stone-tile background contract). Locked style vocabulary: universal preamble + universal palette clause + universal negative-prompt clause, applied verbatim at the head and tail of every prompt. Retroactive fits verified for shipped Warrior, Skeleton, dungeon floor, dungeon wall — zero style gaps. Drift prevention defined as a 5-item PR checklist (block ID citation, prompt transcript, deviation justification, palette audit, silhouette thumbnail). Extension protocol: ≥3-upcoming-assets + body-plan/format differentiation + worked-example-at-lock-time. Open Questions empty at lock. Unblocks ART-03 / ART-12 / ART-13 / ART-14 / ART-07a / ART-07b authoring. |
...
| ART-SPEC-02 | Species sprite generation spec (art half) | To Do | P1 | **Tag-team with SPEC-SPECIES-01.** Art-lead authors the canonical species-sprite pipeline: prompt skeleton (extends ART-SPEC-01's character/monster block), per-species slot table (quadruped / winged / arachnid / bipedal), 8-direction coverage rules, animation set requirements (idle / walk / attack / death), palette-clamp rule vs. per-species tint, scale multiplier per SpeciesConfig. Lives in `docs/assets/species-pipeline.md`. Ships with worked example on one species (proposes Bat or Wolf from ART-14 as the exemplar). Paired with SPEC-SPECIES-01. |
