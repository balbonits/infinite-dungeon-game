...
| SYS-12 | Bank & Backpack Redesign (Guild window) | Impl (milestones 2a–2g done; equipment-on-death pending wiring) | Milestones 1 (spec lock) + 2a–2g done: model rewrite (Inventory/Bank `long` gold, one-type-per-slot, Lock flag, Transfer/Drop), reusable `SlotGrid` + `NumberFormat`, new `GuildWindow` (3 tabs), refreshed `BackpackWindow` with Drop/Lock, 5-option sacrifice death dialog, Guild Maid with dedicated ART-02 sprite, updated test suites. Remaining: wire `EquipmentSet.DestroyRandomEquipped()` into death sacrifice dialog options (Save Backpack / Accept Fate / Quit destroy 1 random equipped item; Save Both costs `deepestFloor × 25`). |
