...
| ART-SPEC-01 | PixelLab prompt template library | Done (spec) | P1 | **Rewritten 2026-04-17 for true-iso / Diablo 1 reference — v1 (commit `375f42e`) superseded.** Locked in [docs/assets/prompt-templates.md](assets/prompt-templates.md). Pivot: engine is true 2:1 iso per SPEC-ISO-01; v1's "low top-down" preamble was wrong and produced the empirical `+Vector2(0,40)` spawn offset ISO-01d removes. Reference: Diablo 1 / Hellfire (Warrior / Rogue / Sorcerer / Catacombs tiles / Arrow 8-dir / Spell Icons) — **inspiration, not port**. Seven named blocks: **CHAR-HUM-ISO** (humanoid chars / classes / NPCs / bipedal monsters, 128×128), **CHAR-MON-ISO** (non-humanoid monsters, `.quad` / `.wing` / `.arach` / `.biped-mon` sub-variants), **TILE-ISO-ATLAS** (floor diamond + N/S/E/W wall faces + corners + T-junctions + cross-junctions + stairs + doors, per-biome sub-clamps for 8 biomes, ~30 slots/biome × 8 = ~240 tiles), **OBJ-ISO** (props bottom-center anchored, 64-wide footprint with variable height), **PROJ-ISO-8DIR** (32×32 projectiles, 8-dir), **ICON-UI-64** (64×64 ability icons, D1 spell-icon frame), **PORTRAIT-NPC** (256×256 2D bust, UI-only), **PORTRAIT-CLASS** (256×384 2D hero art, UI-only). Locked perspective + canvas contract: bottom-center anchor rule with derived `Sprite2D.offset.y = -(H/2) - 16` formula, 8-direction rotation naming mapped to iso screen space, per-family canvas + view-parameter table. Redraw policy: every shipped character / monster / tile / object slated for re-gen; exempt = game logo + game icon + HP orb + MP orb. Drift prevention: 6-item PR checklist (v1's 5 + iso-alignment bullet). Extension protocol: ≥3-assets + body-plan/format/perspective/canvas gate + worked-example-at-lock. Unblocks wave 2 family specs ART-SPEC-02 → ART-SPEC-09. |
...
| ART-SPEC-02 | Species sprite generation spec (art half) | To Do | P1 | **Tag-team with SPEC-SPECIES-01.** Art-lead authors the canonical species-sprite pipeline: prompt skeleton (extends ART-SPEC-01's character/monster block), per-species slot table (quadruped / winged / arachnid / bipedal), 8-direction coverage rules, animation set requirements (idle / walk / attack / death), palette-clamp rule vs. per-species tint, scale multiplier per SpeciesConfig. Lives in `docs/assets/species-pipeline.md`. Ships with worked example on one species (proposes Bat or Wolf from ART-14 as the exemplar). Paired with SPEC-SPECIES-01. **Delete-before-regen: Bucket B in [asset-inventory.md](assets/asset-inventory.md).** |
