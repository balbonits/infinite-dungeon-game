...
| ID | Title | Status | Priority | Notes |
|----|-------|--------|----------|-------|
| AUDIT-01 | `Enemy.cs:53` — `SpawnRateModifier` mixed into enemy HP | Done | P1 | Fixed: removed `intel.SpawnRateModifier` from `hpMult` and `intel.AggressionModifier` from `spdMult` in `Enemy._Ready` (both were stat misuses per their docstrings — `DungeonIntelligence.cs:140, :151`). `SpawnRateModifier` now correctly drives `Dungeon._spawnTimer.WaitTime` (`Dungeon.cs:33`); `AggressionModifier` now correctly drives `Enemy._hitCooldown.WaitTime` (`Enemy.cs:69`). Aggro range remains uniform (current architecture has no aggro range — enemies always chase the player). 470 unit tests green. |
