...
- **Hitstop script:** new `Hitstop` autoload exposing `public static void Trigger(float durationSeconds)`. Internally it sets `Engine.TimeScale = 0` and must use a timer that ignores `TimeScale` to restore it. **Critical deadlock risk:** if `Engine.TimeScale = 0`, a `SceneTreeTimer` created with `GetTree().CreateTimer(seconds)` will itself be frozen and never fire — the game permanently stalls at the hitstop pause. Two safe restoration mechanisms (pick one at impl time):
  1. **Unscaled SceneTreeTimer:** `GetTree().CreateTimer(seconds, processInPhysics: false, processAlways: true, ignoreTimeScale: true)` — the `ignoreTimeScale: true` flag (Godot 4.2+) makes the timer advance in wall-clock time regardless of `Engine.TimeScale`. This is the simplest option if the Godot version supports it. Alternative if targeting pre-4.2: `processAlways: true` keeps the timer ticking during pause, but `Engine.TimeScale = 0` still affects its countdown — do NOT rely on `processAlways` alone here.
  2. **Wall-clock restore in `_Process`:** record `startTick = Time.GetTicksMsec()` when hitstop begins; in `_Process(delta)` (which continues to fire if the autoload has `ProcessMode = Always`), check `(Time.GetTicksMsec() - startTick) / 1000.0 >= duration` and restore `TimeScale = 1.0` when true. Framerate-independent and zero-dependency on Godot's timer implementation details.
  Either way, the `Hitstop` autoload MUST set `ProcessMode = Always` so its `_Process` continues during pause. An integration test that triggers a hitstop and then asserts `TimeScale == 1` after `duration + 1` wall-clock seconds gates this contract — without the test, a regression could silently deadlock the game.
